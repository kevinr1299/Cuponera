			...
			if(cuponRepository.existsById(canjear.getCupon())) {
				...
				if(cupon.getEstadoCupon().getId() == 1) {
					if(cupon.getCliente().getDui().equals(canjear.getDui())) {
						...
						return ResponseEntity.ok("Cupón canjeado");
					}else {
						return ResponseEntity.status(400).body("El DUI del propietario del cupón y el ingresado no coinciden");
					}
				}else if(cupon.getEstadoCupon().getId() == 2){
					return ResponseEntity.status(400).body("El cupón ya esta canjeado");
				}else {
					return ResponseEntity.status(400).body("El cupón ya esta vencido");
				}
			}else {
				return ResponseEntity.status(404).body("No se encontro ningun cupón con ese número");
			}
		}catch(Exception ex) {
			System.out.println(ex);
			return ResponseEntity.status(500).body("No se pudo canjear el cupón");
		}
	}
